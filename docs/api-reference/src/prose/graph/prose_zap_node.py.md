...
- **相对路径**：`src/prose/graph/prose_zap_node.py`
- **模块导入名**：`src.prose.graph.prose_zap_node`
- **代码行数**：35
...
| 函数 | `prose_zap_node` | 23 | `(state: ProseState)` |

## 符号详解

### `logger`

- **类型**：模块常量  |  **行号**：20–20  |  **完整限定名**：`src.prose.graph.prose_zap_node.logger`
- **值**：
...
- **类型**：函数  |  **行号**：23–35  |  **完整限定名**：`src.prose.graph.prose_zap_node.prose_zap_node`
- **签名**：
...
LangGraph 工作流节点函数，对应 `prose_zap` 节点。接收当前 State 与 RunnableConfig，执行该节点的业务逻辑，并返回需要合并回 State 的部分字典或 `Command` 对象。
...
#
# TODO: 结合业务场景补充 prose_zap_node 的典型调用示例。
```
