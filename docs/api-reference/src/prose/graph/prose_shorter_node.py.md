...
- **相对路径**：`src/prose/graph/prose_shorter_node.py`
- **模块导入名**：`src.prose.graph.prose_shorter_node`
- **代码行数**：32
...
| 函数 | `prose_shorter_node` | 22 | `(state: ProseState)` |

## 符号详解

### `logger`

- **类型**：模块常量  |  **行号**：19–19  |  **完整限定名**：`src.prose.graph.prose_shorter_node.logger`
- **值**：
...
- **类型**：函数  |  **行号**：22–32  |  **完整限定名**：`src.prose.graph.prose_shorter_node.prose_shorter_node`
- **签名**：
...
LangGraph 工作流节点函数，对应 `prose_shorter` 节点。接收当前 State 与 RunnableConfig，执行该节点的业务逻辑，并返回需要合并回 State 的部分字典或 `Command` 对象。
...
#
# TODO: 结合业务场景补充 prose_shorter_node 的典型调用示例。
```
